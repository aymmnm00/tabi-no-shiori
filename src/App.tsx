...
.plan-box { background:#F7FBFD; border-radius:12px; padding:10px; margin-top:8px; }
.plan-done { background:#E1F5EC; color:#3F8F6C; font-size:12px; font-weight:700; text-align:center; padding:8px; border-radius:12px; }
.spot-pick-list { max-height:220px; overflow-y:auto; border:1.5px solid #E4E9ED; border-radius:12px; margin-bottom:6px; }
.spot-pick-item { display:flex; align-items:center; gap:8px; width:100%; background:white; border:none; border-bottom:1px solid #F0F4F7; padding:9px 11px; font-size:13px; text-align:left; cursor:pointer; font-family:inherit; color:var(--navy); }
.spot-pick-item:last-child { border-bottom:none; }
.spot-pick-item:active { background:#F3FAFE; }
.spot-pick-name { flex:1; min-width:0; font-weight:700; }
.spot-pick-country { font-size:10.5px; opacity:0.55; }
...
.rate-manual { display:flex; align-items:center; gap:6px; margin-top:6px; }
.rate-manual-label { font-size:12px; font-weight:700; white-space:nowrap; }
...
// 為替レートを取得する(無料・登録不要のAPIを2つ順に試す)
// 1通貨あたり何円かを返す。どちらも失敗すれば null。
async function fetchRate(code) {
  // ① Frankfurter(ECBのデータ)
  try {
    const res = await fetch(`https://api.frankfurter.dev/v1/latest?base=${code}&symbols=JPY`);
    if (res.ok) {
      const data = await res.json();
      if (data?.rates?.JPY) return { value: data.rates.JPY, date: data.date || "" };
    }
  } catch (e) { /* 次を試す */ }

  // ② open.er-api.com(別系統のデータ)
  try {
    const res = await fetch(`https://open.er-api.com/v6/latest/${code}`);
    if (res.ok) {
      const data = await res.json();
      if (data?.rates?.JPY) {
        const d = data.time_last_update_utc
          ? new Date(data.time_last_update_utc).toISOString().slice(0, 10)
          : "";
        return { value: data.rates.JPY, date: d };
      }
    }
  } catch (e) { /* あきらめる */ }
...
function ScheduleForm({ initial, trip, onSave, onCancel }) {
  ...
  const [pickSpot, setPickSpot] = useState(false);   // スポット一覧を開いているか

  // スポットの内容を予定の入力欄に流し込む
  const applySpot = (w) => {
    setTitle(w.name);
    setLocation(w.name);
    if (typeof w.lat === "number") setCoord(`${w.lat}, ${w.lng}`);
  ...
  return (
    <div className="mini-form">
      {(trip.wishlist || []).length > 0 && (
        <>
          <button className="btn-secondary full" type="button" onClick={() => setPickSpot(!pickSpot)} style={{ marginBottom: 4 }}>
            <Heart size={13} style={{ verticalAlign: "-2px", marginRight: 4 }} />
            {pickSpot ? "閉じる" : "スポットから選ぶ"}
          </button>
          {pickSpot && (
            <div className="spot-pick-list">
              {(trip.wishlist || []).map((w) => (
                <button key={w.id} type="button" className="spot-pick-item" onClick={() => applySpot(w)}>
                  <span className="wish-item-icon">{w.icon || "🌐"}</span>
                  <span className="spot-pick-name">{w.name}</span>
                  {w.country && <span className="spot-pick-country">{w.country}</span>}
                </button>
              ))}
            </div>
          )}
...
function WishlistTab({ trip, updateTrip }) {
  ...
  const [planId, setPlanId] = useState(null);                 // 予定に追加しようとしている項目
  const [planDate, setPlanDate] = useState("");
  const [planTime, setPlanTime] = useState("");
  const [planDone, setPlanDone] = useState("");               // 追加できたときの案内

  const tripDates = dateRange(trip.startDate, trip.endDate);

  // スポットを、指定した日の予定として追加する
  const addToSchedule = (w) => {
    ...
    const item = {
      ...
      memo: w.memo || "",
      timeZone: "jst",
      ...(typeof w.lat === "number" ? { lat: w.lat, lng: w.lng } : {}),
    };
    const days = trip.days || {};
    updateTrip({ ...trip, days: { ...days, [planDate]: [...(days[planDate] || []), item] } });
    setPlanId(null); setPlanTime("");
    setPlanDone(`${w.name} を ${fmtDateShort(planDate)} の予定に追加しました`);
    setTimeout(() => setPlanDone(""), 3000);
  };
  ...
  return (
    <div className="tab-content">
      ...
      {planDone && <div className="plan-done">✅ {planDone}</div>}
                ...
                {planId === item.id ? (
                  <div className="plan-box" onClick={(e) => e.stopPropagation()}>
                    <div className="field-row">
                      <div>
                        <label className="field-label">日付</label>
                        <select className="field-input" value={planDate} onChange={(e) => setPlanDate(e.target.value)}>
                          <option value="">選ぶ</option>
                          {tripDates.map((d, i) => (
                            <option key={d} value={d}>Day{i + 1} ({fmtDateShort(d)})</option>
                          ))}
                        </select>
                      </div>
                      <div>
                        <label className="field-label">時刻</label>
                        <input type="time" className="field-input" value={planTime} onChange={(e) => setPlanTime(e.target.value)} />
                      </div>
                    </div>
                    <div className="form-actions">
                      <button className="btn-mini full" disabled={!planDate || !planTime} onClick={() => addToSchedule(item)}>追加する</button>
                      <button className="btn-secondary full" onClick={() => setPlanId(null)}>やめる</button>
                    </div>
                  </div>
                ) : (
                  <button
                    className="link-btn"
                    style={{ paddingLeft: 0 }}
                    onClick={(e) => { e.stopPropagation(); setPlanId(item.id); setPlanDate(tripDates[0] || ""); }}
                  >
                    <Calendar size={11} style={{ verticalAlign: "-1px", marginRight: 3 }} />予定に追加
                  </button>
                )}
              </div>
              <button className="icon-btn faint" onClick={() => setDeletingId(item.id)}><X size={16} /></button>
...
function SplitTab({ trip, updateTrip }) {
  ...
  const refreshRate = async () => {
    ...
    else { setRateError(true); setShowManual(true); }
    setRateBusy(false);
  };

  // 自動で取れないときに、自分でレートを入れる
  const saveManualRate = () => {
    const v = Number(manualRate);
    if (!v) return;
    const today = new Date().toISOString().slice(0, 10);
    updateTrip({ ...trip, rate: { value: v, date: today } });
    setShowManual(false); setRateError(false); setManualRate("");
  };
        ...
        {cur && (
          <>
            <div className="rate-bar">
              ...
              <button className="link-btn" onClick={() => setShowManual(!showManual)}>手入力</button>
            </div>

            {rateError && (
              <div className="field-hint" style={{ color: "#C25B3E", fontWeight: 700 }}>
                自動で取得できませんでした。下の欄に手で入力してください。
              </div>
            )}

            {showManual && (
              <div className="rate-manual">
                <span className="rate-manual-label">1 {cur.code} =</span>
                <input
                  type="number" inputMode="decimal" className="field-input"
                  placeholder="例:165" value={manualRate}
                  onChange={(e) => setManualRate(e.target.value)}
                  style={{ flex: 1, minWidth: 0 }}
                />
                <span className="rate-manual-label">円</span>
                <button className="btn-mini" onClick={saveManualRate}>決定</button>
              </div>
            )}
          </>
        )}
